...
def get_anchors(skips = anchor_skips, base_sizes = anchor_base_sizes, aspect_ratios=anchor_aspect_ratios):
    def base_boxes(l, rs):
    ...
    def cal_feature_boxs(skip, bbs):
        ...
        for i in range(anchor_base_n):
            (LS[i], TS[i]), (RS[i], BS[i]) = np.meshgrid(ls[i], ts[i]), np.meshgrid(rs[i], bs[i])
        CXS, CYS = np.meshgrid(cxs, cys)
        LS, RS, TS, BS = LS.transpose(1, 2, 0), RS.transpose(1, 2, 0), TS.transpose(1, 2, 0), BS.transpose(1, 2, 0)
        CXS, CYS = np.stack(([CXS] * anchor_base_n), axis=-1), np.stack([CYS] * anchor_base_n, axis=-1)
        feature_boxes = np.array([LS, RS, TS, BS, CXS, CYS]).transpose(1, 2, 3, 0).astype(np.int32)
...
anchors = get_anchors(skips = anchor_skips, base_sizes = anchor_base_sizes, aspect_ratios=anchor_aspect_ratios)
