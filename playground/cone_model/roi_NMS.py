...
def NMS(features, box_cred_th = 0.0, cls_crd_th = 0.0, maximumn_obj_num=1):
    ...
    for img_idx in range(features[0].shape[0]):
        ...
        for anchor_layer_idx in range(len(features)):
            ...
            pos_points = np.where((out[::,::,::,0]>box_cred_th)*(cls_confidence_plan>cls_crd_th))
            ...
            frms, tos = 1+classes_num+cls_plane[pos_points]*4, 1+classes_num+cls_plane[pos_points]*4+4
            if len(frms) > 0:
                rls, rrs, rts, rbs = list(np.array([layer_boxes_param[frm:to] for layer_boxes_param, frm, to in zip(layer_boxes_params, frms, tos)]).transpose())
            else:
                rls, rrs, rts, rbs = [], [], [], []
            s, e = cxs + reg_range_d * (ls - cxs), cxs + reg_range_u * (ls - cxs)
            ls = np.minimum(np.maximum(0, (s + (e - s) * rls + reg_range_d)).astype(np.int), IMG_W - 1)
            s, e = cxs + reg_range_d * (rs - cxs), cxs + reg_range_u * (rs - cxs)
            rs = np.minimum(np.maximum(0, (s + (e - s) * rrs + reg_range_d)).astype(np.int), IMG_W - 1)
            s, e = cys + reg_range_d * (ts - cys), cys + reg_range_u * (ts - cys)
            ts = np.minimum(np.maximum(0, (s + (e - s) * rts + reg_range_d)).astype(np.int), IMG_H - 1)
            s, e = cys + reg_range_d * (bs - cys), cys + reg_range_u * (bs - cys)
            bs = np.minimum(np.maximum(0, (s + (e - s) * rbs + reg_range_d)).astype(np.int), IMG_H - 1)
