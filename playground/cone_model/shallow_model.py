...
class Net(nn.Module):
    ...
    def forward(self, imgs, mode='testing', objs = None, roi_samples=None):
        ...
        convF1c_clses = torch.concatenate(convF1c_paral_clses, axis = 1)
        convF1c_coors = torch.concatenate(convF1c_paral_coors, axis = 1)
        ...
        convF1c = torch.concatenate((convF1c_objness, convF1c_clses, convF1c_coors), axis=2)
        ...
        convF2c_clses = torch.concatenate(convF2c_paral_clses, axis = 1)
        convF2c_coors = torch.concatenate(convF2c_paral_coors, axis = 1)
        ...
        convF2c = torch.concatenate((convF2c_objness, convF2c_clses, convF2c_coors), axis=2)
        ...
        convF3c_clses = torch.concatenate(convF3c_paral_clses, axis = 1)
        convF3c_coors = torch.concatenate(convF3c_paral_coors, axis = 1)
        ...
        convF3c = torch.concatenate((convF3c_objness, convF3c_clses, convF3c_coors), axis=2)
        ...
        if len(cls_idcs) > 0:
            ...
            roi_fc2_clses = torch.concatenate([fc[::,:1] for fc in roi_fc2_Y1s], axis=1)
            roi_fc2_coors = torch.concatenate([fc[::,1:] for fc in roi_fc2_Y1s], axis=1)
            roi_fc2_Y1 = torch.concatenate((roi_fc2_clses, roi_fc2_coors), axis=1)
        ...
        if 'training' in mode:
            ...
            for idx, (reg, box) in enumerate(zip(roi_regs, boxes)):
                ...
                angle_code = reg[7:]
                angle_l_code = torch.concatenate((angle_code[1:],angle_code[:1]))
                angle_r_code = torch.concatenate((angle_code[-1:],angle_code[:-1]))
