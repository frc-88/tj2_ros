...
F_thick = (1+classes_num+4*classes_num)*anchor_base_n # 1 for objectiveness, classes_num for classification the rest for coordinates
...
class ParallelConv2d(nn.Module):
    def __init__(self, n_in, n_out, kernel_size, stride, padding, activation):
        ...
        self.convs = nn.ModuleList([nn.Conv2d(n_in, n_out, kernel_size=kernel_size, stride=stride, padding=padding) for i in range(classes_num)])
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.convF1b_paral_clses = ParallelConv2d(n_ch[5], 100, 3, 1, padding=1, activation=F.relu)
        self.convF1c_paral_clses = ParallelConv2d(100, 1 * anchor_base_n, 1, 1, padding=0, activation=torch.sigmoid)
        self.convF1b_paral_coors = ParallelConv2d(n_ch[5], 100, 3, 1, padding=1, activation=F.relu)
        self.convF1c_paral_coors = ParallelConv2d(100, 4 * anchor_base_n, 1, 1, padding=0, activation=torch.sigmoid)
        ...
        self.convF2b_paral_clses = ParallelConv2d(n_ch[4], 100, 3, 1, padding=1, activation=F.relu)
        self.convF2c_paral_clses = ParallelConv2d(100, 1 * anchor_base_n, 1, 1, padding=0, activation=torch.sigmoid)
        self.convF2b_paral_coors = ParallelConv2d(n_ch[4], 100, 3, 1, padding=1, activation=F.relu)
        self.convF2c_paral_coors = ParallelConv2d(100, 4 * anchor_base_n, 1, 1, padding=0, activation=torch.sigmoid)
        ...
        self.convF3b_paral_clses = ParallelConv2d(n_ch[3], 100, 3, 1, padding=1, activation=F.relu)
        self.convF3c_paral_clses = ParallelConv2d(100, 1 * anchor_base_n, 1, 1, padding=0, activation=torch.sigmoid)
        self.convF3b_paral_coors = ParallelConv2d(n_ch[3], 100, 3, 1, padding=1, activation=F.relu)
        self.convF3c_paral_coors = ParallelConv2d(100, 4 * anchor_base_n, 1, 1, padding=0, activation=torch.sigmoid)

        # for roi regression
        self.roi_conv_in = nn.Conv2d(n_roi_ch[0], n_roi_ch[1], 3, 1, padding=1) # Starts the roi branch
        ...
        self.roi_fc1_Y1s = ParallelFC(n_roi_ch[3]*roi_align_h*roi_align_w//(4*4), 1000, activation=F.relu)
        self.roi_fc2_Y1s = ParallelFC(1000, 1+4, activation=torch.sigmoid)
        self.roi_fc1_Y2 = nn.Linear(n_roi_ch[3]*roi_align_h*roi_align_w// (4 * 4), 1000)
    ...
    def forward(self, imgs, mode='testing', objs = None, roi_samples=None):
        ...
        def sub_segments(obj, mode):
            ...
            up8 = F.relu(self.up8(self.upsample8(conv7a[img_idx:img_idx + 1, ::, qt:qb + 1, ql:qr + 1])))
            merge8 = torch.cat((conv2a[img_idx:img_idx + 1, ::, ht:hb + 1, hl:hr + 1], up8), 1)
            conv8 = F.relu(self.conv8(merge8))

            up9 = F.relu(self.up9(self.upsample9(conv8)))
            merge9 = torch.cat((conv1a[img_idx:img_idx + 1, ::, ct:cb + 1, cl:cr + 1], up9), 1)
        ...
        convF1c_clses = torch.cat(convF1c_paral_clses, axis = 1)
        convF1c_coors = torch.cat(convF1c_paral_coors, axis = 1)
        convF1c_objness = convF1c_objness.reshape((convF1c_objness.shape[0], anchor_base_n, -1)+convF1c_objness.shape[-2:])
        convF1c_clses = convF1c_clses.reshape((convF1c_clses.shape[0], anchor_base_n, -1) + convF1c_clses.shape[-2:])
        convF1c_coors = convF1c_coors.reshape((convF1c_coors.shape[0], anchor_base_n, -1) + convF1c_coors.shape[-2:])
        convF1c = torch.cat((convF1c_objness, convF1c_clses, convF1c_coors), axis=2)
        ...
        convF2c_clses = torch.cat(convF2c_paral_clses, axis = 1)
        convF2c_coors = torch.cat(convF2c_paral_coors, axis = 1)
        convF2c_objness = convF2c_objness.reshape((convF2c_objness.shape[0], anchor_base_n, -1)+convF2c_objness.shape[-2:])
        convF2c_clses = convF2c_clses.reshape((convF2c_clses.shape[0], anchor_base_n, -1) + convF2c_clses.shape[-2:])
        convF2c_coors = convF2c_coors.reshape((convF2c_coors.shape[0], anchor_base_n, -1) + convF2c_coors.shape[-2:])
        convF2c = torch.cat((convF2c_objness, convF2c_clses, convF2c_coors), axis=2)
        ...
        convF3c_clses = torch.cat(convF3c_paral_clses, axis = 1)
        convF3c_coors = torch.cat(convF3c_paral_coors, axis = 1)
        convF3c_objness = convF3c_objness.reshape((convF3c_objness.shape[0], anchor_base_n, -1)+convF3c_objness.shape[-2:])
        convF3c_clses = convF3c_clses.reshape((convF3c_clses.shape[0], anchor_base_n, -1) + convF3c_clses.shape[-2:])
        convF3c_coors = convF3c_coors.reshape((convF3c_coors.shape[0], anchor_base_n, -1) + convF3c_coors.shape[-2:])
        convF3c = torch.cat((convF3c_objness, convF3c_clses, convF3c_coors), axis=2)
        ...
        box_cred_th_nms = box_cred_th_nms_train if 'training' in mode else box_cred_th_nms_test
        cls_cred_th_nms = box_cred_cls_confi_th_nms_train if 'training' in mode else box_cred_cls_confi_th_nms_test
        boxes = NMS(features, box_cred_th_nms, cls_cred_th_nms, maximumn_obj_num=maximumn_obj_num)
        roi_train_set = []
        aligns = []
        if mode == 'testing_stage1':
            ...
            for box in boxes:
                ...
                intact, stand, angle = torch.tensor(0).to(device), torch.tensor(1).to(device), torch.tensor(0).to(device)
                mask = np.zeros((bb+1-bt,br-bl+1))
                segments.append([box_img_idx, box_cls_idx, bt, bb, bl, br, intact, stand, angle, mask])
            return segments
        ...
        if 'training' in mode:
            ...
            for box in boxes:
                ...
                for obj_idx, obj in enumerate(objs):
                    iou = get_iou(box, obj) if obj['img_idx'] == box_img_idx and obj['cls_idx'] == box_cls_idx else 0
                    if max_iou < iou:
                        max_iou = iou
                        max_idx = obj_idx
                if max_iou > roi_iou_reg_thresh:
                    ...
                    pos_neg = 1
                    cls_pos_candidates[box_cls_idx].append({'img_idx':box_img_idx, 'cls_idx':box_cls_idx, 'pos_neg':pos_neg, \
                                                            'from_box':from_box, 'to_box':to_box, 'intact':obj['intact'],
                                                            'stand':obj['stand'], 'l_r':obj['l_r'], \
                                                            'angle':obj['angle'], 'for_reg':for_reg, 'for_pos_neg':for_pos_neg, \
                                                            'for_i':obj['for_i'],
                                                            'for_s':obj['for_s'], 'for_lr':obj['for_lr'], 'for_angle':obj['for_angle']})
                elif max_iou < roi_iou_neg_thresh:  # the negative samples from proposals are quite important
                    for_reg, for_pos_neg = 0, 1
                    to_box = {'t':0, 'b':0, 'l':0, 'r':0}# in fact it's not used in this case
                    pos_neg = 0
                    intact, stand, l_r, angle, for_s, for_lr, for_angle, for_i = 0, 0, 0, 0, 0, 0, 0, 0
                    cls_neg_candidates[box_cls_idx].append({'img_idx':box_img_idx, 'cls_idx':box_cls_idx, 'pos_neg':pos_neg, \
                                                            'from_box':from_box, 'to_box':to_box, 'intact':intact,
                                                            'stand':stand, 'l_r':l_r, \
                                                            'angle':angle, 'for_reg':for_reg, 'for_pos_neg':for_pos_neg, \
                                                            'for_i': for_i,
                                                            'for_s':for_s, 'for_lr':for_lr, 'for_angle':for_angle})
            ...
            for obj_idx, obj in enumerate(objs):
                ...
                for sample in roi_samples:
                    ...
                    if obj['img_idx'] == sample['img_idx'] and obj['cls_idx'] == sample['cls_idx'] and obj_idx == sample['obj_idx']:
                        from_box = {'t':sample['ft'], 'b':sample['fb'], 'l':sample['fl'], 'r':sample['fr']}
                        roi_train_set.append({'img_idx':sample['img_idx'], 'cls_idx':sample['cls_idx'], 'pos_neg':sample['pos_neg'],\
                                              'from_box':from_box, 'to_box':to_box, 'intact':sample['intact'],
                                              'stand':sample['stand'], 'l_r':sample['l_r'], 'angle':sample['angle'],
                                              'for_reg':sample['for_reg'], 'for_pos_neg':sample['for_pos_neg'],\
                                              'for_i':sample['for_i'], 'for_s':sample['for_s'], 'for_lr':sample['for_lr'], 'for_angle':sample['for_angle']})
            for sample in roi_train_set:
                ...
                t, b, l, r = sample['from_box']['t'], sample['from_box']['b'], sample['from_box']['l'], sample['from_box']['r']
        ...
        if len(cls_idcs) > 0:
            ...
            roi_fc2_clses = torch.cat([fc[::,:1] for fc in roi_fc2_Y1s], axis=1)
            roi_fc2_coors = torch.cat([fc[::,1:] for fc in roi_fc2_Y1s], axis=1)
            roi_fc2_Y1 = torch.cat((roi_fc2_clses, roi_fc2_coors), axis=1)
            ...
            roi_regs = [reg[idx] for idx, reg in zip(cls_idcs, roi_detect)] # digits in crop_reg are [cls, 4 coors, intact, s]
            roi_regs = [torch.cat((reg,rear)) for reg, rear in zip(roi_regs, roi_fc2_Y2)]
        else:
            roi_regs = []
        ...
        if 'training' in mode:
            ...
            for idx, (reg, box) in enumerate(zip(roi_regs, boxes)):
                ...
                if pos_neg > roi_score_th:
                    reged_boxes.append([img_idx, cls_idx, ttt, bbb, lll, rrr, intact, stand, angle*20])
            for reged_box in reged_boxes:
                img_idx, cls_idx, org_t, org_b, org_l, org_r = reged_box[:6]
                obj = {'img_idx':img_idx, 'cls_idx':cls_idx, 't':org_t, 'b':org_b, 'l':org_l, 'r':org_r}
                ...
                mask = mask[org_t-ct:org_b+1-ct,org_l-cl:org_r+1-cl].cpu().detach().numpy()
                segment = [img_idx, cls_idx, org_t, org_b, org_l, org_r, intact, stand, angle, mask]
                segments.append(segment)
        # the contents of segments are different in training and testing mode
        output = [feature1, feature2, feature3, segments, roi_reg_infos] if 'training' in mode else segments
        return output
