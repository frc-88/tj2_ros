...
class TrapezoidalProfileLive:
    ...
    def calculate_velocity(self, current_position, current_velocity, dt):
        """
        ...
        command_vel = self.calculate_accelerated_vel(current_velocity, forwards, dt)

        # Limit the command velocity based on the max speed
        command_vel = self.apply_max_velocity_limit(command_vel)

        # Limit the command velocity such that it has time to decelerate to hit the target at the right velocity
        command_vel = self.apply_deceleration_limit(command_vel, forwards)
    ...
    def apply_deceleration_limit(self, current_vel, forwards):
        """
        Limit the command velocity such that it has time to decelerate to hit the target at the right velocity.
        """
    ...
    def calculate_command_position(self, current_vel, dt):
        ...
        """
        return self.last_command_pos + dt * current_vel