...
class PowerModeNode:
    def __init__(self) -> None:
        self.default_mode_id = self.mode_str_to_mode(
            rospy.get_param("~default_power_mode", JetsonPowerMode.HIGH_POWER.name),
        )
        self.launch_trigger = self.mode_str_to_mode(
            rospy.get_param("~launch_trigger", JetsonPowerMode.HIGH_POWER.name),
        )
        self.launch_path = str(rospy.get_param("~launch_path", ""))
        self.bypass = bool(rospy.get_param("~bypass", False))
    ...
    def mode_str_to_mode(self, mode_str: str) -> JetsonPowerMode:
        try:
            return JetsonPowerMode[mode_str]
        except KeyError:
            rospy.logerr(f"Invalid power mode: {mode_str}. Defaulting to UNKNOWN")
    ...
    def run(self):
        if self.default_mode_id != JetsonPowerMode.UNKNOWN and not self.bypass:
            self.set_power_mode(self.default_mode_id)
        if self.bypass and self.primary_launcher:
            rospy.loginfo("Bypassing power mode setting. Starting primary launcher")
            self.primary_launcher.start()
        rospy.spin()
