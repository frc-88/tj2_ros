...
NT_TYPE_GET_MAPPING: Dict[ByteString, Callable[[NetworkTableEntry], ConvertedNtValue]] = {
    ...
    NetworkTablesInstance.EntryTypes.BOOLEAN_ARRAY: lambda entry: [bool(x) for x in entry.getBooleanArray([])],
    NetworkTablesInstance.EntryTypes.DOUBLE_ARRAY: lambda entry: [float(x) for x in entry.getDoubleArray([])],
    NetworkTablesInstance.EntryTypes.STRING_ARRAY: lambda entry: [str(x) for x in entry.getStringArray([])],
}
...
NT_TYPE_SET_MAPPING: Dict[ByteString, Callable[[NetworkTableEntry, ConvertedNtValue], bool]] = {
...
NT_TYPE_NAME_DESERIALIZE_MAPPING: Dict[ByteString, Callable[[List[str]], ConvertedNtValue]] = {
...
def recurse_nt(data: TableBackup, current_path: str, table: NetworkTable):
    for entry_name in table.getKeys():
        ...
        data[current_path + "/" + entry_name] = (NT_TYPE_NAME_MAPPING[entry_type], value)
        ...
        with open(path) as file:
            ...
            for index, row in enumerate(reader):
                ...
                if nt_type not in NT_TYPE_NAME_REVERSE_MAPPING:
                    warnings.warn(f"Found invalid NT type '{nt_type}' in {path} on row {index}")
                    continue
                nt_type_code = NT_TYPE_NAME_REVERSE_MAPPING[nt_type]
                ...
                if nt_type_code not in NT_ARRAY_TYPES:
                    if length != 1:
                        warnings.warn(f"Found invalid NT length '{length}' in {path} on row {index}. Should be 1.")
                        continue
