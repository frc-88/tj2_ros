    ...
    nh(*nodehandle), _tf_listener(_tf_buffer)
{
    ...
    string key;
    if (!ros::param::search("send_topics", key)) {
        ROS_ERROR("Failed to find send_topics parameter");
        std::exit(EXIT_FAILURE);
    }
    ROS_DEBUG("Found send_topics: %s", key.c_str());
    nh.getParam(key, _send_topics);

    if (!ros::param::search("receive_topics", key)) {
        ROS_ERROR("Failed to find receive_topics parameter");
        std::exit(EXIT_FAILURE);
    }
    ROS_DEBUG("Found receive_topics: %s", key.c_str());
    nh.getParam(key, _receive_topics);
